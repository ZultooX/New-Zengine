...
void PixelShaderExporter::ExportToBinaryFile(const char* aOutDst, const PixelShader& aVertexShader)
{
}

void PixelShaderExporter::ExportAllToBinaryFile(const char* aOutDst)
{
	std::vector<BinaryExporter::ShaderData> myBinaryData;
	std::vector<BinaryExporter::AssetIndex> myIndex;

	for (const auto& entry : std::filesystem::directory_iterator(ZENGINE_SHADERS_PATH "Pixel"))
	{
